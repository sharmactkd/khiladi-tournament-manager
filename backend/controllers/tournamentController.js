...
const normalizeTieSheetClientSeq = (value) => {
...
const clearScopeEntryIds = Array.isArray(req.body?.clearScopeEntryIds)
  ...
  : [];
    ...
    const updated = await Tournament.findOneAndUpdate(
      ...
      {
        new: true,
        select: "tiesheet tiesheetOutcomeSeq tiesheetOutcomeSavedAt",
      }
    ).lean();
 ...
 const syncResult = await syncTieSheetMedalsToEntries({
  ...
  clearScopeEntryIds,
    ...
    return res.status(200).json({
      ...
      tiesheetOutcomeSavedAt: updated?.tiesheetOutcomeSavedAt || null,
    });
  } catch (error) {
