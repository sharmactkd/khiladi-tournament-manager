...
const validationSchema = Yup.object({
  ...
  ageCategories: Yup.object({
    ...
    .test(
      ...
      'At least one age category must be selected',
      function (value) {
        if (!value) return false;
  ...
  eventCategories: Yup.object({
    kyorugi: Yup.object({
      ...
      sub: Yup.object().when('selected', {
        ...
        then: (schema) =>
          schema.test(
            'at-least-one-kyorugi-sub',
            'At least one Kyorugi sub-event is required',
            function (sub) {
              if (!sub) return false;
    ...
    .test(
      'at-least-one-event',
      'At least one event (Kyorugi or Poomsae) is required',
      function (value) {
        if (!value) return false;
  ...
  // ====== WEIGHT CATEGORIES – PROPER VALIDATION FOR NEW STRUCTURE ======
// src/components/TournamentForm/validationSchema.js

weightCategories: Yup.object({
  type: Yup.string().oneOf(["WT", "SGFI", "custom"]).required(),
  custom: Yup.object().when("type", {
    is: "custom",
    then: (schema) =>
      schema
        .test("has-data", "At least one age group must have divisions", (value) => {
          return value && Object.values(value).some(rows => Array.isArray(rows) && rows.length > 0);
        })
        .test("valid-rows", "Invalid weight divisions", (value) => {
          ...
          for (const rows of Object.values(value)) {
            ...
            for (const row of rows) {
              if (!row.min || !row.max || parseFloat(row.min) >= parseFloat(row.max)) {
                return false;
              }
            }
          }
